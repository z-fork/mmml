...
class ArtQuestionIndexBase(AlgMeta):
    ...
    def get_knowledge_tree(self):
        ...
        for _, row in K.iterrows():
            kid, parent_id, level = map(int, row)
            if level == 1:
                res[kid] = kid
    ...
    def get_question_knowledge_index(self, qfactor, ktree):
        res = set()
        for ktag in qfactor.knowledge_vec:
            if ktag in ktree:
                res.add(ktree[ktag])
        return res
    ...
    def set_knowledge_inv_index(self):
        ...
        for key in keys:
            ...
            k_str, rk_str, diff = self.redis_db.lrange(key, 0, -1)
            k_vec = load_vector(k_str)
            for ktag in k_vec:
