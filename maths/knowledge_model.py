...
def load_question_factor(redis_db, key):
    try:
        k_str, m_str, k_m_str, diff = redis_db.lrange(key, 0, -1)
    except ValueError:
        k_str, m_str, k_m_str, diff = '{}', '{}', '{}', '0'
    diff = float(diff)
    k_mat, m_mat, k_m_mat = map(load_matrix, [k_str, m_str, k_m_str])
    return k_mat, m_mat, k_m_mat, diff


class MathQuestionFactor(object):
