...
def get_record_factor(conn_func, banker_conn_func, parent_kind):
    conn = conn_func()
    ...
    sql = ('select target_kind, target_id, tag_id from knowledge_tag '
           'where target_kind = %s')
    ktags = pd.read_sql_query(sql, banker_conn, params=(qtype,))
    ...
    diff = pd.read_sql_query(sql, banker_conn)
    if len(diff) == 0:
        return None
    ...
    def _convert_fac(x):
        ...
        return "|".join([t+':'+d for t, d in res])
...
def calc_user_factor(conn_func, banker_conn_func):
    records = []
    for parent_kind in CONF_MAP.keys():
        record = get_record_factor(conn_func, banker_conn_func, parent_kind)
...
if __name__ == "__main__":
    ...
    user_factor = calc_user_factor(conn_func, banker_conn_func)
    dump_user_factor(redis_db, user_factor)
    print 'cet user factor done'
