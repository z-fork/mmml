...
from utils.data_store import (
        redis_cet_online, redis_cet_debug, flush_redis,
        get_cet_conn, get_cet_dev_conn,
        get_banker_conn, get_banker_dev_conn
)
from utils.consts import (READING_CONFS, LISTEN_CONFS,
...
def dump_factor(conn, qtype, info, origin):
    res = []
    for qid, factor in info.iteritems():
        ...
        if not origin_info:
            continue
        key = ENGLISH_QUESTION_FACTOR_KEY % (qtype, qid)
...
def get_combine_info1(conn_func, banker_conn_func, redis_db, conf,
...
def get_reading_info(conn_func, banker_conn_func, redis_db,
                     knowledge, related_knowledge, origin):
    res = []
    for conf in READING_CONFS[0]:
        res.extend(get_combine_info1(conn_func, banker_conn_func,
                                     redis_db, conf, knowledge,
                                     related_knowledge, origin))
    for conf in READING_CONFS[1]:
        res.extend(get_combine_info2(conn_func, banker_conn_func,
                                     redis_db, conf, knowledge,
                                     related_knowledge, origin))
    return res


def get_listen_info(conn_func, banker_conn_func, redis_db,
                    knowledge, related_knowledge, origin):
    res = []
...
def get_knowledge_index(redis_db):
    key_pattern = ENGLISH_QUESTION_FACTOR_KEY % ('*', '*')
    keys = redis_db.keys(key_pattern)
    k_idx = {}
    rk_idx = {}
    for key in keys:
        qtype, qid = map(int, key.split('/')[-2:])
        score = redis_db.get(key)
        if not score:
            continue
...
if __name__ == "__main__":
    ...
    remove_unapproved_questions(conn_func, banker_conn_func, redis_db)

    origin = get_origin(banker_conn_func)
    knowledge = get_knowledge(banker_conn_func, 'knowledge_tag')
    related_knowledge = get_knowledge(banker_conn_func, 'related_knowledge_tag')

    reading_idx = get_reading_info(conn_func, banker_conn_func, redis_db,
                                   knowledge, related_knowledge, origin)
    print 'reading ok'
    listen_idx = get_listen_info(conn_func, banker_conn_func, redis_db,
                                 knowledge, related_knowledge, origin)
    print 'listen ok'

    question_index = reading_idx + listen_idx
