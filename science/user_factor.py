...
class ScienceUserFactorBase(AlgMeta):
    ...
    def get_question_record(self):
        ...
        qfactor_dict = {}
        for key in keys:
            kscore, mscore = self.extract_tags(key)
            qid = int(key.split('/')[-1])
        ...
        if len(res) == 0:
            ...
            res.columns = ['uid', 'correct', 'kscore', 'mscore']
        return res
    ...
    def dump_user_factor(self, user_factor):
        ...
        for idx, row in user_factor.iterrows():
            ...
            self.redis_db.rpush(key, kscore, mscore)
...
class PhysicsUserFactor(PhysicsMeta, ScienceUserFactorBase):
    pass


class ChemistryUserFactor(ChemistryMeta, ScienceUserFactorBase):
    pass


class BiologyUserFactor(BiologyMeta, ScienceUserFactorBase):
    pass
...
if __name__ == "__main__":
    ...
    physics_user_factor = PhysicsUserFactor(conn_func, redis_db)
    physics_user_factor.compute_user_factor(args.flush_old_factor)
    print 'physics done.'

    chemistry_user_factor = ChemistryUserFactor(conn_func, redis_db)
    chemistry_user_factor.compute_user_factor(args.flush_old_factor)
    print 'chemistry done.'

    biology_user_factor = BiologyUserFactor(conn_func, redis_db)
    biology_user_factor.compute_user_factor(args.flush_old_factor)
    print 'biology done.'
